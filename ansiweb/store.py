...
def validate(cfg: dict) -> None:
    ...
    for pc in cfg.get("pcs", []):
        if not NAME_RE.match(pc.get("name", "")):
            raise ValidationError(f"PC name '{pc.get('name')}' is not valid (letters, digits, - _ . only)")
    ...
    if helper and helper.lower() not in names:
        raise ValidationError(f"Office helper PC '{helper}' is not in the PC list")
    for key in ("deploy", "office_cache"):
...
def write_inventory(cfg: dict) -> None:
    ...
    helper = (cfg.get("office") or {}).get("helper_pc")
    if helper:
        children["office_helper"] = {"hosts": {helper: None}}
...
def write_connection_vars() -> None:
    text = """# Generated by AnsiWEB. How Ansible connects to the Windows PCs.
