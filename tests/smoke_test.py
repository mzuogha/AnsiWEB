...
r = c.post("/settings", data={"csrf":tok,"server_ip":"not-an-ip","forks":"20","batch_size":"20","cc_hours":"24","dep_time":"19:00"}, follow_redirects=True)
...
# every page renders
for url in ["/","/apps","/apps/new","/apps/7zip/edit","/pcs","/pcs/PC-HQ-001/edit","/jobs","/settings"]:
    r = c.get(url); assert r.status_code == 200, (url, r.status_code, r.text[:500])
assert c.get("/office").status_code == 404, "Office page should be gone"
